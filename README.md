...
AgentQ status and diagnostics are coordination triage, not answer-quality evidence. When the user asks why an answer was bad, whether AgentQ helped, or to review recent agent behavior, the correct artifact is a qualitative review of actual messages, replies, handoff text, or same-prompt before/after outputs. Do not turn that request into a queue/status summary, score, or new work-frame report.
...
- Do not use `status`, `diag`, counts, stale-work totals, or queue summaries as the answer to a quality/process complaint. Inspect the actual conversation first, then mention AgentQ state only if it changed the user-visible conclusion.
